...
pub fn interpret(payload: Option<&str>) -> PurchaseOutcome {
    ...
    let tokens = crate::json::enumerate_tokens(Some(payload));
    let lowercase = payload.to_ascii_lowercase();

    if lowercase.contains("\"alreadyowned\":true")
        || tokens
            .iter()
            .any(|token| crate::json::try_read_boolean(token, "alreadyOwned") == Some(true))
    {
    ...
    if lowercase.contains("\"success\":true")
        || tokens
            .iter()
            .any(|token| crate::json::try_read_boolean(token, "success") == Some(true))
    {
